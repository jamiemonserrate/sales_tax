...
describe LineItem do
  ...
  context '#taxes' do
    it 'should calculate the total of taxes for the given quantity' do
      expect(LineItem.new(2, Product.new('imported wine', 10.00)).taxes).to eq(3.00)
    end
  end

  context '#price_after_tax' do
    it 'should calculate the total price after tax for the given quantity' do
      expect(LineItem.new(2, Product.new('imported wine', 10.00)).price_after_tax).to eq(23.00)
    end
  end
end